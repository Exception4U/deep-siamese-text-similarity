...
tf.flags.DEFINE_string("training_files", "/media/root/Heavy_Dataset/gta_data/final/", "training folder")
tf.flags.DEFINE_integer("hidden_units", 50, "Number of hidden units in softmax regression layer (default:50)")
tf.flags.DEFINE_string("word2vec", "","Google's pretrained embedding")
...
tf.flags.DEFINE_string("conv_layer_weight_pretrained_path", "/media/root/Heavy_Dataset/amos/data1.npy", "AMOSNet pre-trained weights path")
...
inpH = InputHelper()
train_set, dev_set, sum_no_of_batches = inpH.getDataSets(FLAGS.training_files, max_document_length, 10, FLAGS.batch_size)
...
with tf.Graph().as_default():
    ...
    loss_summary = tf.scalar_summary("loss", siameseModel.loss)
    acc_summary = tf.scalar_summary("accuracy", siameseModel.accuracy)
    ...
    def train_step(x1_batch, x2_batch, y_batch):
        ...
        _, step, loss, accuracy, dist, sim, summary = sess.run([tr_op_set, global_step,\
                                                          siameseModel.loss,\
    ...
    def dev_step(x1_batch, x2_batch, y_batch):
        ...
        step, loss, accuracy, dist, sim, summary = sess.run([global_step, \
                                               siameseModel.loss,\
                                               siameseModel.accuracy,\
                                               siameseModel.distance,\
                                               siameseModel.temp_sim,\
