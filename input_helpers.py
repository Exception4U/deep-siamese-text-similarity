...
class InputHelper(object):
    ...
    def getTsvData(self, base_filepath, max_document_length):
        ...
        l = []
        for line in open(base_filepath + 'test2.txt'):
        #for line in open('ultra_simple_positive_annotations'):
            if (line[0] == 'F'):
                if ('/' in line):
                    line = line.split("/")[0]
                l.append(line.strip())
        ...
        #for line in open('negative_annotations.txt'):
            line=line.split('/', 1)[0]
    ...
    def getTsvTestData(self, base_filepath, max_document_length):
        ...
        # Loading Positive sample file
        l = []
        for line in open(base_filepath + 'unipolar_annotations.txt'):
            line=line.split('/', 1)[0]
            if (len(line) > 0  and line[0] == 'F'):
                l.append(line.strip())
